...
@APP.post("/api/business-trip")
async def business_trip(data: business_trip.Data,
                        nc: typing.Annotated[AsyncNextcloudApp, Depends(anc_app)]):
    user = await nc.user
    ...
    employee = data.employee if data.employee else ''
    if employee == 'SELF':
        employee = user_fullname_gen
    ...
    context = {
        ...
        'organization': data.organization if data.organization else '',
        'summary': data.summary if data.summary else '',
        'transport': data.transport if data.transport else '',
        'auto_number': data.auto_number if data.auto_number else '',
    }

    # Формируем путь к шаблону заявления.
    # Шаблон должен быть расположен в общем каталоге файлов "Шаблоны/Заявления/заявление_на_отпуск_бс.docx"
    _fn = f"{NC_DOC_TEMPLATES_DIR}/служебная_записка_на_командировку.docx"
